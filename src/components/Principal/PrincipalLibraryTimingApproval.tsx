...
const PrincipalLibraryTimingApproval: React.FC = () => {
  const { currentUser } = useAuth();
  const db = getDatabase();
  
  const [requests, setRequests] = useState<LibraryTimingRequest[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedRequest, setSelectedRequest] = useState<LibraryTimingRequest | null>(null);
  ...
  const fetchRequests = async () => {
    try {
      ...
      if (snapshot.exists()) {
        const allRequests: LibraryTimingRequest[] = [];
        snapshot.forEach((childSnapshot) => {
          const request = childSnapshot.val();
          // Show requests that are pending Principal approval or were created by Principal
          if (request.currentApproverRole === 'principal' || request.createdBy === currentUser?.uid) {
            allRequests.push({
    ...
    try {
      ...
      const updatedHistory = [
        ...request.history,
        {
          ...
          by: (currentUser as any)?.displayName || 'Principal',
          role: 'principal',
          timestamp: new Date().toISOString(),
          comment: comment || 'Request approved by Principal'
        }
      ];

      const updates: any = {};
      updates[`libraryTimingRequests/${requestId}/status`] = 'approved';
      updates[`libraryTimingRequests/${requestId}/currentApproverRole`] = 'completed';
      updates[`libraryTimingRequests/${requestId}/approvalFlow`] = updatedApprovalFlow;
      updates[`libraryTimingRequests/${requestId}/history`] = updatedHistory;
    ...
    try {
      ...
      const updatedHistory = [
        ...request.history,
        {
          ...
          role: 'principal',
          timestamp: new Date().toISOString(),
          comment: comment || 'Request rejected by Principal'
        }
      ];

      const updates: any = {};
      updates[`libraryTimingRequests/${requestId}/status`] = 'rejected';
      updates[`libraryTimingRequests/${requestId}/currentApproverRole`] = 'completed';
      updates[`libraryTimingRequests/${requestId}/approvalFlow`] = updatedApprovalFlow;
      updates[`libraryTimingRequests/${requestId}/history`] = updatedHistory;
  ...
  const filteredRequests = requests.filter(request => {
    ...
    const matchesFilter = filter === 'all' || request.priority === filter;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent mb-2">
            Library Timing Request Approval
          </h1>
          <p className="text-gray-600 dark:text-gray-400">
            Review and approve library timing change requests
          </p>
        </div>
            ...
            filteredRequests.map((request) => (
              <div key={request.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  {request.status === 'pending' && request.currentApproverRole === 'principal' && (
...
export default PrincipalLibraryTimingApproval; 