...
const TeacherApplicationReview: React.FC = () => {
  const { user } = useAuth();
    ...
    const unsubscribe = onValue(applicationsRef, (snapshot) => {
      ...
      if (data) {
        ...
        Object.keys(data).forEach((key) => {
          ...
          // Show applications that are pending registrar review or forwarded to registrar
          if (application.currentLevel === 'registrar' || 
              (application.status === 'pending' && application.currentLevel === 'registrar') ||
              application.status === 'approved_by_hod' ||
              application.status === 'forwarded_to_registrar') {
  ...
  const handleApprove = async (applicationId: string) => {
    ...
    try {
      ...
      await update(applicationRef, {
        ...
        approvedByRegistrar: user.uid,
        registrarApprovalTime: Date.now(),
        registrarComment: comment || 'Approved by Registrar'
      });

      // Create notification for principal
      const notificationRef = ref(database, 'notifications');
      await push(notificationRef, {
        userId: 'principal', // Will be sent to all principals
        title: 'Teacher Application Requires Final Review',
        message: `A teacher application has been approved by Registrar and forwarded to Principal for final review.`,
  ...
  const handleReject = async (applicationId: string) => {
    ...
    try {
      ...
      await update(applicationRef, {
        ...
        rejectedByRegistrar: user.uid,
        registrarRejectionTime: Date.now(),
        registrarComment: comment || 'Rejected by Registrar'
      });
  ...
  const handleForward = async (applicationId: string) => {
    ...
    try {
      ...
      await update(applicationRef, {
        ...
        currentLevel: 'principal',
        forwardedByRegistrar: user.uid,
        registrarForwardTime: Date.now(),
        registrarComment: comment || 'Forwarded to Principal for final review'
      });
      ...
      await push(notificationRef, {
        ...
        message: `A teacher application has been forwarded by Registrar to Principal for final review.`,
        type: 'teacher_application',
        applicationId: applicationId,
        timestamp: Date.now(),
        read: false
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'rejected_by_registrar':
        return <span className="px-2 py-1 bg-red-100 text-red-800 rounded-full text-xs">Rejected by Registrar</span>;
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">Teacher Application Review</h1>
        <p className="text-gray-600">Review and manage teacher applications - Approve, Reject, or Forward to Principal</p>
      </div>
                ...
                <tr key={application.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    {(application.status === 'pending' || 
                      application.status === 'approved_by_hod' || 
                      application.status === 'forwarded_to_registrar' ||
                      application.currentLevel === 'registrar') && (
