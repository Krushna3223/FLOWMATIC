...
import { AlertCircle, CheckCircle, Clock, FileText, Users, Filter, Search, ArrowRight } from 'lucide-react';
...
const HostelComplaintReview: React.FC = () => {
          ...
          snapshot.forEach((studentSnapshot) => {
            studentSnapshot.forEach((complaintSnapshot) => {
              allComplaints.push({
                ...
                ...complaintSnapshot.val()
              });
            });
          });
          
          // Filter for complaints that need registrar review
          const registrarComplaints = allComplaints.filter(
            complaint => complaint.status === 'forwarded_to_registrar' || 
                        (complaint.currentLevel === 'registrar' && complaint.status === 'pending')
          );
          
          setComplaints(registrarComplaints.sort((a, b) => 
  ...
  const handleAction = async (complaintId: string, action: 'approve' | 'reject', comments?: string) => {
    try {
  ...
  const stats = {
    ...
    pending: complaints.filter(c => c.status === 'forwarded_to_registrar').length,
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-3xl font-bold text-gray-800 mb-2">Hostel Complaint Review</h1>
              <p className="text-gray-600">Review complaints that need registrar approval</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-blue-50 p-4 rounded-lg">
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  <p className="text-sm text-blue-600">Total Forwarded</p>
          ...
          <div className="space-y-4">
            <h3 className="text-xl font-semibold mb-4">Forwarded Complaints ({filteredComplaints.length})</h3>
            ...
            {loading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-600">No forwarded complaints found</p>
              </div>
            ) : (
              <div className="space-y-4">
                {filteredComplaints.map((complaint) => (
                  <div key={complaint.id} className="bg-white border border-gray-200 rounded-lg p-6">
                    ...
                    {/* Hostel Rector Comments */}
                    {complaint.workflow?.hostel_rector?.comments && (
                      <div className="mb-4">
                        <p className="text-sm text-gray-600 mb-2">Hostel Rector Comments:</p>
                        <p className="text-gray-800 bg-blue-50 p-3 rounded-lg">{complaint.workflow.hostel_rector.comments}</p>
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {complaint.status === 'forwarded_to_registrar' && (
                        <div className="flex space-x-2">
                          <button
                            onClick={() => {
      ...
      {showDetails && selectedComplaint && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold">Final Review - Hostel Complaint</h2>
              <button
                onClick={() => {
                  setShowDetails(false);
                  setSelectedComplaint(null);
            ...
            <div className="space-y-4">
              ...
              {selectedComplaint.workflow?.hostel_rector?.comments && (
                <div>
                  <p className="text-sm text-gray-600">Hostel Rector Comments</p>
                  <p className="text-gray-800 bg-blue-50 p-3 rounded-lg">{selectedComplaint.workflow.hostel_rector.comments}</p>
                </div>
              )}
...
export default HostelComplaintReview; 