...
export default function CivilSupervisorMaintenanceRequests() {
    ...
    try {
      ...
      const unsubscribe = onValue(requestsRef, (snapshot) => {
        if (snapshot.exists()) {
          ...
          // Filter requests that are pending and at civil supervisor level
          const civilSupervisorRequests = requests.filter(req => 
            req.status === "pending" && req.currentLevel === "civil_supervisor"
          );
          
          requests.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
          setMaintenanceRequests(civilSupervisorRequests);
        } else {
          console.log('⚠️ CivilSupervisorMaintenanceRequests: No maintenance requests found in Firebase');
  ...
  const handleApprove = async (request: MaintenanceRequest) => {
    try {
      const updatedData = {
        ...request,
        status: "approved_by_civil_supervisor",
        currentLevel: "civil_supervisor",
        nextLevel: "registrar",
        updatedAt: new Date().toISOString(),
        timeline: [
          ...request.timeline,
          {
            level: "civil_supervisor",
            action: "Approved and work completed",
            timestamp: new Date().toISOString(),
            user: currentUser?.name || "Civil Supervisor"
          }
        ]
      };
      ...
      const notificationData = {
        ...
        title: 'Civil Maintenance Completed',
        message: `Your civil maintenance request "${request.title}" has been completed by Civil Supervisor`,
  ...
  const handleReject = async (request: MaintenanceRequest) => {
    ...
    try {
      ...
      const notificationData = {
        ...
        title: 'Civil Maintenance Request Rejected',
        message: `Your civil maintenance request "${request.title}" has been rejected by Civil Supervisor: ${reason}`,
        requestId: request.id,
        rejectionReason: reason,
        status: 'unread',
        createdAt: new Date().toISOString(),
  ...
  const getStatusColor = (status: string) => {
    const colors: { [key: string]: string } = {
      ...
      "approved_by_civil_supervisor": "bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400",
  ...
  const stats = {
    ...
    approved: maintenanceRequests.filter(r => r.status === "approved_by_civil_supervisor").length,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Civil Maintenance Requests</h2>
          <p className="text-gray-600 dark:text-gray-400">Review and manage civil maintenance requests from students</p>
        </div>
      </div>
      ...
      <div className="flex space-x-1 bg-gray-100 dark:bg-gray-800 rounded-xl p-1">
        {[
          ...
          { id: "approved_by_civil_supervisor", name: "Completed", count: stats.approved },
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="text-center py-8">
              ...
              <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-300 mb-2">
                No Civil Maintenance Requests
              </h3>
              <p className="text-gray-500 dark:text-gray-400">
                {activeTab === "all" ? "No civil maintenance requests assigned to you yet." : `No ${activeTab} requests found.`}
              </p>
            </div>
          </div>
        ) : (
