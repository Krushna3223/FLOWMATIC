...
export default function ElectricianMaintenanceRequests() {
    ...
    try {
      ...
      const unsubscribe = onValue(requestsRef, (snapshot) => {
        if (snapshot.exists()) {
          ...
          // Filter requests that are pending and at electrician level
          const electricianRequests = requests.filter(req => 
            req.status === "pending" && req.currentLevel === "electrical_technician"
          );
          
          requests.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
          setMaintenanceRequests(electricianRequests);
        } else {
          console.log('⚠️ ElectricianMaintenanceRequests: No maintenance requests found in Firebase');
  ...
  const handleApprove = async (request: MaintenanceRequest) => {
    try {
      const updatedData = {
        ...request,
        status: "approved_by_electrician",
        currentLevel: "electrical_technician",
        nextLevel: "registrar",
        updatedAt: new Date().toISOString(),
        timeline: [
          ...request.timeline,
          {
            level: "electrical_technician",
            action: "Approved and work completed",
            timestamp: new Date().toISOString(),
            user: currentUser?.name || "Electrical Technician"
          }
        ]
      };
      ...
      const notificationData = {
        ...
        title: 'Electrical Maintenance Completed',
        message: `Your electrical maintenance request "${request.title}" has been completed by Electrical Technician`,
  ...
  const handleReject = async (request: MaintenanceRequest) => {
    ...
    try {
      ...
      const notificationData = {
        ...
        title: 'Electrical Maintenance Request Rejected',
        message: `Your electrical maintenance request "${request.title}" has been rejected by Electrical Technician: ${reason}`,
        requestId: request.id,
        rejectionReason: reason,
        status: 'unread',
        createdAt: new Date().toISOString(),
  ...
  const getStatusColor = (status: string) => {
    const colors: { [key: string]: string } = {
      ...
      "approved_by_electrician": "bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400",
  ...
  const stats = {
    ...
    approved: maintenanceRequests.filter(r => r.status === "approved_by_electrician").length,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Electrical Maintenance Requests</h2>
          <p className="text-gray-600 dark:text-gray-400">Review and manage electrical maintenance requests from students</p>
        </div>
      </div>
      ...
      <div className="flex space-x-1 bg-gray-100 dark:bg-gray-800 rounded-xl p-1">
        {[
          ...
          { id: "approved_by_electrician", name: "Completed", count: stats.approved },
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="text-center py-8">
              ...
              <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-300 mb-2">
                No Electrical Maintenance Requests
              </h3>
              <p className="text-gray-500 dark:text-gray-400">
                {activeTab === "all" ? "No electrical maintenance requests assigned to you yet." : `No ${activeTab} requests found.`}
              </p>
            </div>
          </div>
        ) : (
